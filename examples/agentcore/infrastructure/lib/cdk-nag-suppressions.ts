...
export class CdkNagSuppressions {
  ...
  private static suppressAwsManagedPolicies(stack: Stack): void {
    ...
    const lambdaRolePaths = [
      `/${stack.stackName}/AgentUser/SetAgentUserPasswordProviderHandler/ServiceRole/Resource`,
      `/${stack.stackName}/AgentUser/SetAgentUserPasswordProvider/framework-onEvent/ServiceRole/Resource`,
  ...
  private static suppressCustomResourceWildcardPermissions(stack: Stack): void {
    // Suppress for AgentUser custom resource
    NagSuppressions.addResourceSuppressionsByPath(
      stack,
      `/${stack.stackName}/AgentUser/SetAgentUserPasswordProvider/framework-onEvent/ServiceRole/DefaultPolicy/Resource`,
      [
        {
          ...
          appliesTo: [
            "Resource::<AgentUserSetAgentUserPasswordProviderHandler823C4ECB.Arn>:*",
          ],
        },
      ]
    );
  ...
  private static suppressSecretsManagerRotation(stack: Stack): void {
    // Suppress for AgentUser credentials
    NagSuppressions.addResourceSuppressionsByPath(
      stack,
      `/${stack.stackName}/AgentUser/AgentUserCredentials/Resource`,
      [
        {
          id: "AwsSolutions-SMG4",
          reason:
            "The secret contains static agent user credentials for OAuth authentication between " +
            "agent.py and mcp_server.py. Automatic rotation is not required for these service " +
